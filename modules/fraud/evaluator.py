...
from config import (
    ...
    REWARD_TRUE_POSITIVE, BEST_MODEL_PATH, MODEL_REPORT_PATH
)
...
class FraudEvaluator:
    ...
    def evaluate(self, name: str, y_true, y_prob) -> Dict:
        ...
        logger.info(
            ...
            f"Recall={result['recall']} | Caught={tp} | Missed={fn} | "
            f"FalseAlarms={fp} | BusinessValue=${cost:,}"
        )
        return result

    def select_best(self, results: list) -> Dict:
        best = max(results, key=lambda x: x["auc_roc"])
        logger.info(f"Best model: {best['name']} with AUC={best['auc_roc']}")
        return best

    def save_report(self, best: Dict, feature_count: int, test_size: int, test_fraud: int):
        report = {
            ...
            "recall": best["recall"],
            "mcc": best["mcc"],
            "fraud_caught": best["tp"],
            "fraud_missed": best["fn"],
            "false_alarms": best["fp"],
            "business_value": best["business_value"],
            "test_transactions": test_size,
            "test_fraud_total": test_fraud,
